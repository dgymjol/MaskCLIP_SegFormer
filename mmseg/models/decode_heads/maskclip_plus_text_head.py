...
class MaskClipPlusTextHead(BaseDecodeHead):

    def __init__(self, decode_module_cfg, text_categories, text_channels,
                    text_embeddings_path, text_features_path, tau=None, cls_bg=False, norm_feat=False, 
        ...
        super(MaskClipPlusTextHead, self).__init__(
        ...
        try : 
            ...
            raise RuntimeError(f'There is no text_features. ({text_features_path})')
        self.tau = tau
        self.num_classes = text_categories
        
    def init_weights(self, call_super=True):
        if call_super:
            super(MaskClipPlusTextHead, self).init_weights()
    ...
    def _freeze(self):
        ...
        super(MaskClipPlusTextHead, self)._freeze()
    ...
    def cls_seg(self, feat, class_weights):
        """Classify each pixel."""
        ...
        if class_weights is not None:
            batch_size, num_classes, w, h = output.shape
            batch_class_weights = class_weights.unsqueeze(2).repeat(1, 1, w*h).reshape(batch_size, num_classes, w, h)
            output = torch.mul(batch_class_weights, output)
    ...
    def forward(self, inputs, class_weights): 
        output = self.decode_module.forward_module(inputs)

        feat = output.detach()
        output = self.cls_seg(output, class_weights)
    ...
    def forward_train(self, inputs, img_metas, gt_semantic_seg, train_cfg, img=None):
        ...
        if self.tau is None:
            ...
            with torch.no_grad():
                ...
                class_weights = self.num_classes * (((image_features @ self.text_features.T) / self.tau).softmax(dim=-1))

        if self.distill: # False
            seg_logits, feat = self.forward(inputs, class_weights)
            x = self.clip(img)[-1]
            if self.vit:
                v = None
        ...
        elif self.train_unlabeled: # True
            seg_logits, feat = self.forward(inputs, class_weights) #(batchsize, 60, 120, 120) (batch_size, 1024, 120, 120)
            ...
            seg_logits = self.forward(inputs, class_weights)
            losses = self.losses(seg_logits, gt_semantic_seg)
    ...
    def forward_test(self, inputs, img_metas, test_cfg):
        ...
        return self.forward(inputs, class_weights)[0]