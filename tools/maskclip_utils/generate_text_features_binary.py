...
def binary_classifier(model_name, classnames, templates):
    model, preprocess = clip.load(model_name)
    with torch.no_grad():
        class_text_features = []
        for classname in classnames:
            texts = [f"this is a photo of {classname}", f"this is not a photo of {classname}"]
            texts = clip.tokenize(texts).cuda() #tokenize
            text_features = model.encode_text(texts) #embed with text encoder
            text_features = text_features / text_features.norm(dim=1, keepdim=True)
            class_text_features.append(text_features)
    return torch.stack(class_text_features,dim=0)
...
if __name__ == '__main__':
    ...
    binary_weights = binary_classifier(name_mapping[args.model], classes, prompt_templates)

    print(binary_weights.shape)
    
    prefix = f'pretrain/{all_set_name}_{args.model}'

    torch.save(binary_weights, f'{prefix}_clip_binary_text_features.pth')
